...
/**
// Needed if I didn't use auto-init. See explanation in main() below
...
UWORD colortable[] = { 0x00, 0xf44, 0x4f4, 0x44f };
...
VOID doLayers(struct Layer *msgLayer, struct Layer *layer_array[])
{
        ...
        for (ktr = 0; ktr < 3; ktr++)
        {
                if (!SizeLayer(DUMMY, layer_array[ktr], SCREEN_W - (layer_array[ktr]->bounds.MaxX) - 1, 0))
                        error(msgLayer, "SizeLayer() failed.");
        }
...
**
** Return FALSE on failure. On a FALSE return, the layers are properly cleaned up.
*/
...
/*
** Free the bitmap and all bitplanes create by allocBitMap()
*/
...
VOID startLayers(struct Layer_Info *theLayerInfo, struct BitMap *theBitMap)
{
        ...
        if (NULL != (msgLayer = CreateUpfrontLayer(theLayerInfo, theBitMap, M_L, M_T, M_R, M_B, LAYERSMART, NULL)))
        {
                ...
                if (NULL != (theSuperBitMap = allocBitMap(SCREEN_D, SUPER_W, SUPER_H)))
                {
                        ...
                        disposeBitMap(theSuperBitMap, SCREEN_D, SCREEN_W, SUPER_H);
                }
...
**
** See graphics primitives chapter for details on creating and using the low-level graphics calls.
*/
...
VOID main(int argc, char **argv)
{
        ...
        /** 
        // Here is what I would have to do if I didn't use proto headers
        // (other than declaring GfxBase and LayersBase)
