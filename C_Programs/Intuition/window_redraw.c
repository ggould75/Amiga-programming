...
int main(int argc, char **argv)
{
    ...
    if (useGZZ) {
        windowTagItems[16].ti_Data = TRUE;   // 16 == WA_GimmeZeroZero
    }
    if (useBorderless) {
        windowTagItems[17].ti_Data = TRUE;   // 17 == WA_Borderless
    }
    ...
    while (message = (struct IntuiMessage *)GetMsg(window->UserPort))
    {
        ...
        switch (message->Class)
        {
            case IDCMP_REFRESHWINDOW:
                ...
                // Acknowledge the refresh so Intuition can repaint the window
                // frame (borders + gadgets), then redraw our own content.
                BeginRefresh(window);
